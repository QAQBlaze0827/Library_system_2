...
class User {
    ...
    public String toCsvRow() {
        return uid + "," + uname + "," + upassword+ "," + borrowedBooks.stream().map(Book::getBookID).map(String::valueOf).collect(Collectors.joining(";"));
    }
    //解析 CSV 文件中的行
    public static User fromCsvRow(String csvRow){
        ...
        // 如果有借閱書籍，則解析書籍 ID
        if(parts.length>3 && !parts[3].length().isEmpty()){
            List<Book> borrowedBooks=Arrays.stream(parts[3].split(";")).map(Integer::parseInt).map(Book::new).collect(Collectors.toList());
